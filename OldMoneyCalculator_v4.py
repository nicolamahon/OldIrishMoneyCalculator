...
def print_instructions():
	print ("Values must be entered as follows:\n\t> Pounds.Shillings.Pence ---> 3.11.6\n\t> Pence --------------------> 237")
...
def print_menu():
	...
	print ("3: Multiply")
	print ("4: Divide")
	print ("5: Convert Pounds to Pence")
	print ("6: Convert Pence to Pounds")
...
def get_value(option):
	if option == 1 or option == 2: # add / subtract
		...
		value5 = input("\nEnter the pounds value: ")
		return value5
	else: # option == 6
		value6 = int(inputNumber("\nEnter the pence value: "))
		return value6
...
def add(add_values):
	...
	add_total = convertToPounds(added_value)
	return add_total
...
def subtract(subtract_values):
	...
	subtract_total = convertToPounds(subtracted_value)
...
def multiply(multiply_values):
	...
	multiply_total = convertToPounds(multiplied_value)
...
def divide(divide_values):
	...
	divided_total = convertToPounds(divided_value)
...
def main():	
	...
	# confirm user has selected valid menu option
		...
		if option == 1:
			...
			print_header("Convert Pounds to Pence")
			pounds_value = get_value(option)
			pence_value = convertToPence(pounds_value)
			print_pence(pence_value)
		elif option == 6:
			print_header("Convert Pence to Pounds")
			pence_value = get_value(option)
			pound_value = convertToPounds(pence_value)
			print_old_money(pound_value)
		else:
			_ = system('cls')
	# terminate program
	else:
